...
manager.py is a file for tracking how many laps both the opponent and ego complete and how many of those laps the ego was ahead. It also logs distance between the cars, speed of each cars, and the x and y of each car. For each session it now writes two result files in `results/session_<id>/`: `full_session_data_<id>.csv` and `overtaking_events.csv`. There is also a reset function that can reset the cars back to their original position by typing r in the terminal.
